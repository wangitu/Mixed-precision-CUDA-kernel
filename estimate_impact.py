...
def main(
    # model/data params
    base_model="/data/wangqianle/models/llama2-7b",
    data_path="processed_data/c4_processed_10k",
    output_file="processed_data/cherry_indices/llama2-7b-impact.pt",
    ...
    for batch in tqdm(dataloader, desc="Estimating Hessian"):
        ...
        for n, p in model.named_parameters():
            if n.endswith(suffix):
                ...
                n = n[:-len(".weight")]
                if n not in H:
                    H[n] = torch.zeros(grad.shape[-1], dtype=grad.dtype).cuda()
                else:
                    H[n] *= num_batches / (num_batches + 1)
                    
                h = (grad ** 2).mean(dim=tuple(range(grad.ndim - 1)))
